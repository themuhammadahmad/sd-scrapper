...
export default async function puppeteerParser(url) {
  try {
    console.log("📞 Using shared browser for URL:", url);
    console.log("📊 Current stats:", puppeteerManager.getStats());
    
    const content = await puppeteerManager.fetchWithPuppeteer(url);
    
    // Log stats after fetch
    console.log("📊 Stats after fetch:", puppeteerManager.getStats());
    
    return content;
  } catch (error) {
    console.error("❌ Error in puppeteerParser:", error.message);
    throw error;
  }
}